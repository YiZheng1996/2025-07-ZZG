...
namespace MainUI.MVB
{
    public partial class frmPortManager : UIForm
    {
        ...
        private void frmPortManager_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt = modelbll.GetAllKind();
            cboModelName.DisplayMember = "Name";
            cboModelName.ValueMember = "ID";
            cboModelName.DataSource = dt;
            LoadData();
            dataGridView1.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(185, 209, 234);
        }

        void LoadData()
        {
            var lst = bll.GetAllPorts(cboModelName.SelectedValue.ToInt());
        ...
        private void btnDelPort_Click(object sender, EventArgs e)
        {
            ...
            if (r == DialogResult.OK)
            {
                ...
                string port = txtPort.Text;
                bll.DelPortHost(id, "Ports");
                bll.DelPortFullTags(ModelID, port, "FullTags");
                LoadData();
        ...
        private void cboModelName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var lst = bll.GetAllPorts(modelID);
            dataGridView1.DataSource = lst;
        }
    }
}
