...
{
    public class TestStepBLL : BaseBLL
    {
        public TestStepBLL() : base(VarHelper.Database, VarHelper.ConnectionString, "TestStep") { }

        public DataTable GetListByModel(string model)
        {
            return this.GetList("model='" + model + "' order by index");
        }
        ...
        {
            string sql = string.Format("select max([index]) from TestStep where model='{0}';", model);
        ...
        public List<TestStep> GetStep(string model)
        {

            //string sql = string.Format("select a.* from TestStep a left join TestProcess b on a.ProcessKey=b.ProcessKey where a.Model='{0}' ", model);


            string sql = string.Format("select a.* from TestStep a  where a.Model='{0}'", model);//and a.IsVisible='1'
            DataTable dt = this.GetDataTable(sql);

            TestProcessBLL bll = new TestProcessBLL();
            List<TestProcess> processes = bll.GetAll();


            List<TestStep> lst = new List<TestStep>();
            foreach (DataRow row in dt.Rows)
            {
                TestStep step = GetModel(row);
                lst.Add(step);
            }
            return lst;
        ...
        {
            string sql2 = "delete from teststep where model='" + modelID + "'";
        ...
        private void ClearSteps(string model, string repairing, string testType)
        {
            ...
            string sql2 = "delete from teststep where Model='" + model + "' ";
        ...
        public void SaveSteps(List<TestStep> step, string model)
        {
            ...
            {
                TestStep s = step[i];
                string sql = $"insert into TestStep(model,sort,step,ProcessName,ProcessKey,LineNumber,IsVisible,DSLName,FlipToTheLine,ModelID) values('{model}',{s.Sort},{s.Step},'{s.ProcessName}','{s.ProcessKey}',{s.LineNumber},'{s.IsVisible}','{s.DSLName}','{s.ReportRow}',{s.ModelID})";
                this.ExecuteNonQuery(sql);
            }
        }

        public static TestStep GetModel(DataRow row)
        {
            TestStep p = new TestStep();
        ...
        public List<TestStep> GetStepByModelID(string modelID)
        {
            string sql = string.Format("select * from TestStep where Model='{0}' order by sort", modelID); //and a.IsVisible='1'
            DataTable dt = this.GetDataTable(sql);
